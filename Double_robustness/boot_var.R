boot_var_AIPW <- function(df,ps1= working_ps1, ps0=working_ps0,mu01= working_mu01,mu12= working_mu12,mu02=
                           working_mu02, iter) {
  n <- nrow(df)
  ...
  for (i in 1:iter) {
    ...
    est_psi <-  m_estimate(ee_old, data = temp, root_control = setup_root_control(start = c(0,0,0)))@"estimates"
...
boot_var_rbs <- function(df,ps1= working_ps1, ps0=working_ps0,mu01= working_mu01,mu12= working_mu12,mu02=
                            working_mu02, iter) {
  n <- nrow(df)
  est_psi <- data.frame(matrix(NA, nrow = iter, ncol = 3))
  for (i in 1:iter) {
    ...
    est_psi <-  m_estimate(ee_g_estimator, data = temp, root_control = setup_root_control(start = c(0,0,0)))@"estimates"
...
boot_var_gmm <- function(df,ps1= working_ps1, ps0=working_ps0,mu01= working_mu01,mu12= working_mu12,mu02=
  working_mu02, iter) {
  n <- nrow(df)
  est_psi <- data.frame(matrix(NA, nrow = iter, ncol = 3))
  for (i in 1:iter) {
