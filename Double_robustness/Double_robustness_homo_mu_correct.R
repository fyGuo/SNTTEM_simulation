...
est_old <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  ...
  vcov <- boot_var_gmm(df,ps1 = working_ps1, ps0 = working_ps0, mu01 = working_mu01, mu12 = working_mu12, mu02 = working_mu02, iter = 500)
  chisq_stat <- t(est_psi- c(psi01 ,psi02,psi12 )) %*% solve(vcov) %*% (est_psi -  c(psi01 ,psi02,psi12 ))
...
est_g_estimator <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  ...
  vcov <- boot_var_rbs(df,ps1 = working_ps1, ps0 = working_ps0, mu01 = working_mu01, mu12 = working_mu12, mu02 = working_mu02, iter = 500)
  chisq_stat <- t(est_psi- c(psi01 ,psi02,psi12 )) %*% solve(vcov) %*% (est_psi -  c(psi01 ,psi02,psi12 ))
  reject <- chisq_stat > qchisq(0.95, df = 3)
  est <- data.frame(id = i, est_psi01 = est_psi[1], est_psi02 = est_psi[2],est_psi12 = est_psi[3],
                    reject = reject)
