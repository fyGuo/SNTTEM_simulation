...
est_old <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  df <- generate_data(n, psi01 = psi01, psi02 = psi02, psi12 = psi12, p_I1 = p_I1,  ps1_int = ps1_int, delta = delta)
...
est_gmm <- furrr::future_map_dfr(.x = 1:iter,
                                 .f = function(i, n= sample) {
                                   df <- generate_data(n, psi01 = psi01,p_I1 = p_I1, psi02 = psi02, psi12 = psi12, ps1_int = ps1_int, delta = delta)
                                   # generate a sample splitting id
                                   id_1 <- sample(1:n, size = n/2, replace = FALSE)
                                   id_2 <- setdiff(1:n, id_1)
...
df <- generate_data(sample, psi01 = psi01, psi02 = psi02, psi12 = psi12, p_I1 = p_I1, ps1_int = ps1_int, delta = delta)
ps1 <- mean(df$A1)
...
temp <- rbind(est_old, est_g_estimator, est_gmm)
temp$p_I1 <- p_I1
