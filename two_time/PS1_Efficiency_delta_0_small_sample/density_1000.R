...
# draw distributions of psi02

est_ls$est_psi02 %>% mean()
est_ls$est_psi02 %>% sd()

est_gmm$est_psi02 %>% mean()
est_gmm$est_psi02 %>% sd()


est_gmm$method <- "gmm"
est_gmm$est_psi02 <- (est_gmm$est_psi02-mean(est_gmm$est_psi02))/sd(est_gmm$est_psi02)
est_ls$est_psi02 <- (est_ls$est_psi02-mean(est_ls$est_psi02))/sd(est_ls$est_psi02)

est_ls$method <- "ls"
tmp <- rbind(est_gmm, est_ls)



# draw a density of both est_ls and est_gmm for psi02, and draw a standard normal as the benchmark
ggplot(tmp, aes(x = est_psi02, fill = method, color = method)) +
  geom_density(alpha = 0.4, size = 1) +
  stat_function(fun = dnorm, args = list(mean = 0, sd =1),
                color = "black", size = 1) +
  labs(
    x = expression((hat(psi)[0][2] - psi[0][2]) / (sd(hat(psi)[0][2]) )),
    y = "Probability Density (PDF)",
    title = expression("Distribution of Standardized Estimators for " ~ hat(psi)[0][2])
  ) +
  theme_bw()
