...
working_model <- function(data, id_1, id_2,
  ...
  if (mu12 == TRUE){
    mu12_model_1 <- glm(Y2 ~  L0 + A0:L0 , data = filter(data[id_1,], A1==A0), family = binomial(link = "log"),
                        start = c(-3, 1,1))
    mu12_model_2 <- glm(Y2 ~  L0 + A0:L0 , data = filter(data[id_2,], A1==A0), family = binomial(link = "log"),
                        start = c(-3, 1,1))
  } else {
