...
working_model <- function(data, id_1, id_2,
  ...
  if (mu12 == TRUE){
    mu12_model_1 <- glm(Y2 ~ A0 +  L0 + A0:L0 , data = filter(data[id_1,], A1==A0), family = binomial(link = "log"))
    mu12_model_2 <- glm(Y2 ~ A0 +  L0 + A0:L0 , data = filter(data[id_2,], A1==A0), family = binomial(link = "log"))
  } else {
    mu12_model_1 <- glm(Y2 ~ 1, data = filter(data[id_1,], A1==A0), family = binomial(link = "log"))
    mu12_model_2 <- glm(Y2 ~ 1, data = filter(data[id_2,], A1==A0), family = binomial(link = "log"))
  }
  # do the predictions
  data$mu12 <- NA
  ...
  data$dr_mu12 <- NA
  data$dr_mu12[id_1] <-  (data$A0[id_1]==data$A1[id_1])/((1-predict(ps1_model_2, data[id_1,], type = "response"))*(1-data$A0[id_1]) +
                                                          predict(ps1_model_2, data[id_1,], type = "response")*data$A0[id_1])*
    (data$Y2[id_1] - predict(mu12_model_2, data[id_1,], type = "response")) + predict(mu12_model_2, data[id_1,], type = "response")
  data$dr_mu12[id_2]  <- (data$A0[id_2]==data$A1[id_2])/((1-predict(ps1_model_1, data[id_2,], type = "response"))*(1-data$A0[id_2]) +
                                                          predict(ps1_model_1, data[id_2,], type = "response")*data$A0[id_2])*
    (data$Y2[id_2] - predict(mu12_model_1, data[id_2,], type = "response")) + predict(mu12_model_1, data[id_2,], type = "response")
