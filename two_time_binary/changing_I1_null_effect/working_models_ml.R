...
working_model <- function(data, id_1, id_2,
  ...
  if (mu12 == TRUE){
    data$mu12 <- NA
    data[id_2,]$mu12 <- SuperLearner(data1[data1$A1==data1$A0,]$Y2,
                                     data.frame(L1 = data1[data1$A1==data1$A0,]$L1,
                                                A0 = data1[data1$A1==data1$A0,]$A0,
                                                L0 = data1[data1$A1==data1$A0,]$L0),
                                     newX = data.frame(L1 = data2$L1,
                                                       A0 = data2$A0,
                                                       L0 = data2$L0),
                                     family = binomial(),
                                     SL.library = c(rf$names, xgboot$names, gam$names))$SL.predict
    data[id_1,]$mu12 <- SuperLearner(data2[data2$A1==data2$A0,]$Y2,
                                     data.frame(L1 = data2[data2$A1==data2$A0,]$L1,
                                                A0 = data2[data2$A1==data2$A0,]$A0,
                                                L0 = data2[data2$A1==data2$A0,]$L0),
                                     newX = data.frame(L1 = data1$L1,
                                                       A0 = data1$A0,
                                                       L0 = data1$L0),
                                     family = binomial(),
  ...
  data$dr_mu12 <- NA
  data1$dr_mu12 <-  data$dr_mu12[id_1] <-  (data$A0[id_1] == data$A1[id_1])/((1-data[id_1,]$ps1)*(1-data[id_1,]$A0) + data[id_1,]$ps1*data[id_1,]$A0)*(data$Y2[id_1] -data$mu12[id_1]) + data$mu12[id_1]
  data2$dr_mu12 <-  data$dr_mu12[id_2]  <-  (data$A0[id_2] == data$A1[id_2])/((1-data[id_2,]$ps1)*(1-data[id_2,]$A0) + data[id_2,]$ps1*data[id_2,]$A0)*(data$Y2[id_2] -data$mu12[id_2]) + data$mu12[id_2]
