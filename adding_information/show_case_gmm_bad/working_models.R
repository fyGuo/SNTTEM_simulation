...
working_model <- function(data, id_1, id_2,
  ...
  if (mu12 == TRUE){
    mu12_model_1 <- glm(Y2 ~ A0 + L1 + L0 + A0:L0 + Y1, data = filter(data[id_1,], A1==0), family = gaussian(link = "identity"))
    mu12_model_2 <- glm(Y2 ~ A0 + L1 + L0 + A0:L0 + Y1, data = filter(data[id_2,], A1==0), family = gaussian(link = "identity"))
  } else {
  ...
  if (mu01 == TRUE){
    mu01_model_1 <- glm(Y1 ~  L0 , data = filter(data[id_1,], A0==0), family = gaussian(link = "identity"))
    mu01_model_2 <- glm(Y1 ~  L0, data = filter(data[id_2,], A0==0), family = gaussian(link = "identity"))
  } else {
    mu01_model_1 <- glm(Y1 ~ 1, data = filter(data[id_1,], A0==0), family = gaussian(link = "identity"))
    mu01_model_2 <- glm(Y1 ~ 1, data = filter(data[id_2,], A0==0), family = gaussian(link = "identity"))
  }
  # do the predictions
  data$mu01 <- NA
