                                 ...
                                 .f = function(i, n= sample) {
                                   ...
                                   g <- function(theta, x){
                                     cbind((x[,"A0"]-x[,"ps0"])*(((1-x[,"A1"])/(1-x[,"ps1"]))*(x[,"Y"]-x[,"mu1"])+x[,"mu1"]-x[,"mu0"]-gamma0(x[,"L0"], theta[1:3])*x[,"A0"]),
                                           ...
                                           (1- x[,"ps0"])/(1+x[,"ps0"])*(x[,"A1"]-x[,"ps1"])*(x[,"Y"]-gamma1(x[,"L1"], theta[4])*x[,"A1"] - x[,"mu1"]),
                                           (x[,"A1"]-x[,"ps1"])*(x[,"Y"]-gamma1(x[,"L1"], theta[4])*x[,"A1"] - x[,"mu1"]) ) %>%
                                       return()
                                   }
                                   mod <- gmm(g, x = x, t0 = c(0,0,0,0), type = "twoStep")
...
ee_ls_estimator <- function(data){
  function(theta){
    with(data,
         c( (A0-ps0)*((1-A1)/(1-ps1)*(Y-mu1)+mu1-mu0-gamma0(L0, theta[1:3])*A0) + alpha*(1-ps0)/(1+ps0)*(A1-ps1)*(Y-gamma1(L1, theta[4])*A1 - mu1),
            (1+L0)*(A0-ps0)*((1-A1)/(1-ps1)*(Y-mu1)+mu1-mu0-gamma0(L0, theta[1:3])*A0) + alpha*(1-ps0)/(1+ps0)*(A1-ps1)*(Y-gamma1(L1, theta[4])*A1 - mu1),
            exp(L0)*(A0-ps0)*((1-A1)/(1-ps1)*(Y-mu1)+mu1-mu0-gamma0(L0, theta[1:3])*A0) + alpha*(1-ps0)/(1+ps0)*(A1-ps1)*(Y-gamma1(L1, theta[4])*A1 - mu1),
            (A1-ps1)*(Y-gamma1(L1, theta[4])*A1 - mu1)))
  }
}
...
est_ls <- furrr::future_map_dfr(.x = 1:iter,
                                .f = function(i, n= sample) {
                                  ...
                                  phi_1_bar_der <- cbind(mean((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*df$L1*df$A1),
                                                         mean((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*df$L1*df$A1),
                                                         mean((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*df$L1*df$A1))

                                  phi_1_der <- mean((df$A1-df$ps1)*df$L1*df$A1)
                                  ratio <- phi_1_bar_der/phi_1_der
                                  phi_1_bar_resd <- cbind(
                                    ((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*(df$Y-gamma1(df$L1, est[4])*df$A1 - df$mu1)),
                                    ((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*(df$Y-gamma1(df$L1, est[4])*df$A1 - df$mu1)),
                                    ((1-df$ps0)/(1+df$ps0)*(df$A1-df$ps1)*(df$Y-gamma1(df$L1, est[4])*df$A1 - df$mu1))
                                  )
...
est_old <- est_old %>%
  summarise(theta1_mean = mean(theta1, na.rm = TRUE),
            ...
            blip_down_max = max(blip_down, na.rm = TRUE))
