...
temp <- list(est_old, est_g_estimator, est_gmm, est_ls)
names(temp) <- c("Conventiaonl AIPW", "SNTTEM_g_estimator", "GMM", "LS")
temp$ps1 <- ps1
temp$p_I1 <- p_I1

saveRDS(temp, paste0("simulation", sample, ".rds"))

# make histograms to compare the standardized psi02 with a standard normal distribution
est_all <- bind_rows(est_old %>% mutate(method = "Conventiaonl AIPW"),
                     est_g_estimator %>% mutate(method = "SNTTEM g-estimator"),
                     est_gmm %>% mutate(method = "GMM"),
                     est_ls %>% mutate(method = "LS"))
ggplot(est_all, aes(x = est_psi02, fill = method)) +
  geom_density()+
  stat_function(fun = dnorm, args = list(mean = psi02, sd = sd(est_all$est_psi02)), color = "black", size = 1) +
  facet_wrap(~method) +
  labs(title = "Histogram of standardized psi02 estimates by different methods",
       x = "Standardized psi02 estimates",
       y = "Density") +
  theme_bw()
