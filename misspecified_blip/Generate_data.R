...
generate_data <- function(n, psi01, psi02, psi12, ps1_int = 1, delta = 0) {
  I0 <- 1
  L0<-rnorm(n, 0, 1)
  ...
  Y2<-rnorm(n,I1*(1+L0 + psi02*(0.5*A0 + L0*A0 + L1)+psi12*(A1+L1*A1))+
              (1-I1)*(1+L0 + psi02*(0.5*A0 + L0*A0 + L1)+psi12*(100*A1+L1^2*A1)),
            sqrt(1+delta*A1))
  id <- 1:n
...
# here we make gamma12 to be wrongly specified as an example
## blip function at time 1 for variable Y2
gamma12 <- function(L1, psi12, model) {
  # model == 0 means correct
  if (model == 0){
    return( psi12*(1+L1) )
  } else if (model == 1){
    return(psi12)
  } else if (model == 2){
    return(psi12*exp((4*L1)))
  }
}
