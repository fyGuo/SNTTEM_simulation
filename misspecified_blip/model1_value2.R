...
library(matrixcalc)
library(gmm)
setwd("~/PhD_thesis/Thesis_1_SNTTEM/Codes/misspecified_blip")
  ...
  function(theta){
    with(data,
         c(L0*(A0-ps0)*(Y1-gamma01(L0, theta[1])*A0-mu01),
           ...
           I1*L1*(A1-ps1)*(Y2-gamma12(L1, theta[3],  model = 1)*A1 - mu12)))
  }
}
...
est_old <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  ...
  ee_score <-  cbind(df$L0*(df$A0-df$ps0)*(df$Y1-gamma01(df$L0, psi01)*df$A0 - df$mu01),
                     ...
                     df$L1*df$I1*(df$A1-df$ps1)*(df$Y2-gamma12(df$L1, psi12, model =1)*df$A1 - df$mu12))
...
ee_g_estimator <- function(data){
  function(theta){
    with(data,
         c(L0*(A0-ps0)*(Y1-gamma01(L0, theta[1])*A0-mu01),
           L0*(A0-ps0)*(((1-A1)/(1-ps1))^(1-I1)*(Y2-gamma12(L1, theta[3],  model=1)*A1-mu12)+mu12-gamma02(L0, theta[2])*A0-mu02),
           I1*L1*(A1-ps1)*(Y2-gamma12(L1, theta[3], model = 1)*A1 - mu12)))
  }
}
...
est_g_estimator <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  ...
  ee_score <-  cbind(df$L0*(df$A0-df$ps0)*(df$Y1-gamma01(df$L0, psi01)*df$A0 - df$mu01),
                     df$L0*(df$A0-df$ps0)*(((1-df$A1)/(1-df$ps1))^(1-df$I1)*(df$Y2-gamma12(df$L1, psi12, model = 1)*df$A1-df$mu12) + df$mu12-gamma02(df$L0,psi02)*df$A0-df$mu02),
                     df$L1*df$I1*(df$A1-df$ps1)*(df$Y2-gamma12(df$L1, psi12, model = 1)*df$A1 - df$mu12))
                                 ...
                                 .f = function(i, n= sample) {
                                   ...
                                   g <- function(theta, x){
                                     cbind(x[,"L0"]*(x[,"A0"]-x[,"ps0"])*(x[,"Y1"]-gamma01(x[,"L0"], theta[1])*x[,"A0"]-x[,"mu01"]),
                                           ...
                                           x[,"L0"]*(x[,"A0"]-x[,"ps0"])*(((1-x[,"A1"])/(1-x[,"ps1"]))^(1-x[,"I1"])*(x[,"Y2"]-gamma12(x[,"L1"], theta[3],  model = 1)*x[,"A1"] - x[,"mu12"]) + x[,"mu12"] - gamma02(x[,"L0"], theta[2])*x[,"A0"] - x[,"mu02"]),
                                           x[,"I1"]*x[,"L1"]*(x[,"A1"]-x[,"ps1"])*(x[,"Y2"]-gamma12(x[,"L1"], theta[3],  model = 1)*x[,"A1"] - x[,"mu12"])
                                           ) %>%
                                       return()
                                   }
