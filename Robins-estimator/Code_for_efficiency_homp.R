...
ee_iter_projection <- function(data) {
  function(theta){
    with(data,
         c((1+L0)*(A0-ps0)*((1-A1)/(1-ps1)*(Y-mu1)+mu1-mu0-gamma0(L0, theta[1])*A0) +
             (1+L0)*d1*(A1-ps1)*(Y-gamma1(L1, theta[2])*A1 - mu1),
           (1+L1)/(1+2*(varY_control + varY_control^2)*(1-ps1))*(A1-ps1)*(Y-gamma1(L1, theta[2])*A1 - mu1)))
  }
}
...
est_iter_projection <- furrr::future_map_dfr(.x = 1:iter, .f = function(i, n= sample) {
  df <- generate_data(n, psi0 = psi[1], ps1_control, psi1 = psi[2], theta = 0, varY_control = 0)
...
temp$old_eff[i] <-est_old$psi0_eff
temp$SNTTEM_eff[i] <-est_SNTTEM$psi0_eff
temp$ps1[i] <- est_old$ps1
}

ggplot(temp, aes(x = ps1)) +
  geom_line(aes(y = old_eff, color = "Standard estimator")) +
  geom_point(aes(y = old_eff, color = "Standard estimator")) +
  geom_line(aes(y = SNTTEM_eff, color = "Robins' g-estimator")) +
  geom_point(aes(y = SNTTEM_eff, color = "Robins' g-estimator")) +
  scale_color_manual(values = c("Robins' g-estimator" = "blue", "Standard estimator" = "red")) +
  theme_minimal()+
  labs(x = "Pr[A1 = 1]",
       y = "Efficiency",
       color = "Estimator")
