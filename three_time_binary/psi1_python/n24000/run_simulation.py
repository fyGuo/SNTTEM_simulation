"""Outer loop: run simulation over a grid of (p_I1, p_I2, ph1, ph2) values.

n = 24000 configuration for the psi = 1 mechanism, using the psi0-style
INDEPENDENT per-time-point grid (p_I1, p_I2, ph1, ph2) -- 36 cells.
Results are saved to simulation_results.pkl (and .csv for inspection).
"""
import os
import time
import itertools
import pandas as pd

from simulation_output import run_simulation

# True parameter values (psi = 1 mechanism)
PSI03 = 1.0
PSI13 = 1.0
PSI23 = 1.0

# Simulation settings
N = 24000
N_ITER = 300
SEED = 3411

# Grid (matches psi0_python): independent per-time-point parameters
#   ph1, ph2:   per-time-point probability that A_t stays the same as A_{t-1}
#   p_I1, p_I2: per-time-point probability of being eligible (I=1)
P_I1_VALUES = [0.2, 1]
P_I2_VALUES = [0.2, 1]
PH1_VALUES = [0.2, 0.5, 0.8]
PH2_VALUES = [0.2, 0.5, 0.8]

output_path = os.path.join(os.path.dirname(__file__), "simulation_results.pkl")

combos = list(itertools.product(
    P_I1_VALUES, P_I2_VALUES, PH1_VALUES, PH2_VALUES
))
n_combos = len(combos)

all_results = []
combo_times = []
t_start = time.time()

for k, (p_I1, p_I2, ph1, ph2) in enumerate(combos, start=1):
    print(f"\n{'='*60}")
    print(f"[{k}/{n_combos}] Running: p_I1={p_I1}, p_I2={p_I2}, "
          f"ph1={ph1}, ph2={ph2}")
    print(f"{'='*60}")
    t0 = time.time()

    est = run_simulation(
        n=N,
        n_iter=N_ITER,
        psi03=PSI03,
        psi13=PSI13,
        psi23=PSI23,
        p_I1=p_I1,
        p_I2=p_I2,
        ph1=ph1,
        ph2=ph2,
        seed=SEED,
        n_jobs=-1,
    )
    all_results.append(est)
    elapsed = time.time() - t0
    combo_times.append(
        {"p_I1": p_I1, "p_I2": p_I2, "ph1": ph1, "ph2": ph2,
         "seconds": elapsed}
    )
    print(f"Done in {elapsed:.1f}s")

results = pd.concat(all_results, ignore_index=True)
results.to_pickle(output_path)
results.to_csv(output_path.replace(".pkl", ".csv"), index=False)

# --- Timing report ---
times_df = pd.DataFrame(combo_times)
times_path = output_path.replace(".pkl", "_timings.csv")
times_df.to_csv(times_path, index=False)

total = time.time() - t_start
print(f"\n{'='*60}\nTIMING REPORT  ({N_ITER} iterations per combination)\n{'='*60}")
print(times_df.to_string(index=False))
print(f"\nAverage simulation time per combination: "
      f"{times_df['seconds'].mean():.1f}s "
      f"(min {times_df['seconds'].min():.1f}s, "
      f"max {times_df['seconds'].max():.1f}s)")
print(f"Total elapsed over {n_combos} combinations: {total:.1f}s")
print(f"Saved results to {output_path}")
print(f"Saved timings to {times_path}")
