...
def ee_three_step(theta, df):
    ...
    eq2 = np.sum(
        I1 * (A1 - ps1) * (
            w2 * (Y - mu23)
            + mu23 * np.exp(-gamma13(L1, psi13) * (A1 - A0))
            - mu13
        )
    )
    eq3 = np.sum(
        I2 * (A2 - ps2) * (Y * np.exp(-gamma23(L2, psi23) * (A2 - A1)) - mu23)
    )
...
def ee_robins(theta, df):
    ...
    eq1 = np.sum(
        (A0 - ps0) * (
            w2_I * w1_I * (Y * np.exp(-gamma23(L2, psi23) * (A2 - A1)) - mu23)
            + w1_I * (mu23 * np.exp(-gamma13(L1, psi13) * (A1 - A0)) - mu13)
            + mu13 * np.exp(-gamma03(L0, psi03) * A0)
            - mu03
        )
    )
    eq2 = np.sum(
        I1 * (A1 - ps1) * (
            w2_I * (Y * np.exp(-gamma23(L2, psi23) * (A2 - A1)) - mu23)
            + mu23 * np.exp(-gamma13(L1, psi13) * (A1 - A0))
            - mu13
        )
    )
