# Blip (γ) Misspecification Study

This folder reuses the **exact DGP and estimators from `../` (the n24000 run)** but
lets the g-estimators fit with a **deliberately wrong blip functional form**, while
the data are still generated under the **true** blips. It is a template you can copy
into other settings (other sample sizes, other blip forms, non-zero ψ, other grids).
...
- **Fitting (misspecified):** the estimating equations may instead use the wrong forms
  - `gamma13_wrong(L1, ψ) = ψ·(L1 + 10)/(L1 + 4) + 5`
  - `gamma23_wrong(L2, ψ) = ψ·(L2 + 10)/(L2 + 4) + 5`
  - `gamma03` is always kept at its true form.
  - Note the additive `+5` is **not** multiplied by ψ, so the fitted blip is `5`
    even at ψ = 0 and cannot be undone by rescaling ψ (see the caveat below).
...
- **Truth:** `ψ03 = ψ13 = ψ23 = 0`, `N = 24000`, `N_ITER = 300`, `seed = 3411`
  (identical to the n24000 run).

> **Important — the additive `+5` breaks null-invariance.** The blip enters the
> estimating equations as `exp(−γ(L, ψ)·(A − A_prev))`. With a purely multiplicative
> wrong blip `ψ·h(L)`, `γ = 0` at ψ = 0 regardless of `h`, so under the null the wrong
> and true blips give identical equations and there is *no bias* (only a
> variance/identification effect — that was the previous `ψ·(L+10)/(L+4)` design).
>
> The current wrong blips add a constant `+5` that is **not** proportional to ψ. At
> ψ = 0 the fitted blip is `γ = 5` (not 0), so the estimating equation is genuinely
> different from the truth and the solver must move ψ away from 0 to compensate. It
> lands near
>
> > ψ̂ ≈ −5 / E[h(L)] ≈ −5 / 2.85 ≈ −1.75
>
> (since `h(L) = (L+10)/(L+4) ≈ 2.85` over `L ∈ [−1.5, −0.5]`). So **this
> misspecification produces a large bias in the directly-affected parameter even under
> ψ = 0** — γ₁₃_wrong biases ψ̂₁₃, γ₂₃_wrong biases ψ̂₂₃ — and it cannot be absorbed by
> rescaling ψ. (`γ₀₃` stays correct, so ψ̂₀₃ is affected only indirectly, via eq. 1.)
>
> Numerical note: `γ ≈ 8` makes `exp(−γ·ΔA)` large, so the run logs
> `RuntimeWarning: overflow in exp` during the solver's search (harmless) and a few
> replicates may exceed the `|ψ̂| > 10` divergence threshold; the robust summary
> flags/drops those.
...
def gamma13_wrong(L1, psi13):
    ...
    return psi13 * (L1 + 10) / (L1 + 4) + 5   # +5 not multiplied by psi


def gamma23_wrong(L2, psi23):
    """MISSPECIFIED blip at time 2 (true form: psi23 * (L2 + 1))."""
    return psi23 * (L2 + 10) / (L2 + 4) + 5   # +5 not multiplied by psi
```
