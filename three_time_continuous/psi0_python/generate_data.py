...
def generate_data(n, psi03, psi13, psi23, p_I=0.5, ph=0.05, rng=None):
    ...
    I1 = rng.binomial(1, p_I, n)
    L1 = rng.normal(-0.5 * A0, 1, n)
    L1 = np.clip(L1, -1.5, -0.5)
    # A1 = A0 with prob ph, else 1-A0
    A1 = np.where(rng.binomial(1, ph, n) == 1, A0, 1 - A0)

    I2 = rng.binomial(1, p_I, n)
    L2 = rng.normal(-0.5 * A1, 1, n)
    L2 = np.clip(L2, -1.5, -0.5)
    A2 = np.where(rng.binomial(1, ph, n) == 1, A1, 1 - A1)

    log_p = (
        -1.5
        + psi03 * L0 * A0
        + I1 * psi13 * (1 + L1) * (A1 - A0)
        + (1 - I1) * psi13 * L1 / 3 * (A1 - A0)
        + I2 * psi23 * (1 + L2) * (A2 - A1)
        + (1 - I2) * psi23 * L2 / 3 * (A2 - A1)
    )
