...
Same production grid as ../ML_nuisance/run_simulation.py (n=72000, ph in
{0.3,0.5,0.8}, p_I in {0.2,1}) but every nuisance (ps0..ps4, mu05..mu45) is
the EXACT closed-form value implied by the known DGP (oracle_nuisances.py) --
no ML or parametric model is fit from data at all, for any of the three
estimators. This isolates each estimating equation's own finite-sample
bias/variance from nuisance-estimation error entirely; see CLAUDE.md and
oracle_nuisances.py's docstring.
...
# Grid: same as ../ML_nuisance/run_simulation.py (held common across t = 1..4;
# see CLAUDE.md for why a full 2^4 * 3^4 per-time-point factorial isn't used).
P_I_VALUES = [0.2, 1]
