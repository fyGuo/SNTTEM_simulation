#!/bin/bash
#
# Launch the five-time-point SNTTEM simulation, ORACLE variant (psi1,
# n=144000, HARD CELL ONLY (p_I=0.2, ph=0.3), 300 iterations, all three
# estimators fitted with exact closed-form nuisances -- no ML, no
# cross-fitting).
#
#   ./run.sh
#
...
cd "$(dirname "$0")"
HERE="$(pwd -P)"
ROOT="$(cd "$HERE/../../.." && pwd -P)"       # .../five_time_binary (oracle/ is nested one level deeper than n72000/)
...
main() {
    echo "=================================================================="
    echo "SNTTEM five-time-point simulation (psi1, ORACLE nuisances)"
    echo "=================================================================="
    echo "started     : $(date)"
    echo "host        : $(hostname)"
    echo "directory   : $HERE"
