"""Outer loop: run the ORACLE simulation, HARD CELL ONLY, at n=144000.

Same hard cell as ../ML_nuisance/run_simulation.py (p_I=0.2, ph=0.3, n=144000)
but every nuisance (ps0..ps4, mu05..mu45) is the EXACT closed-form value
implied by the known DGP (oracle_nuisances.py) -- no ML or parametric model
is fit from data at all, for any of the three estimators. Run alongside the
ML-nuisance version to separate "is n=144000 enough to stabilize this cell
at all" (oracle) from "...enough given nuisance-estimation noise too"
(ML_nuisance); see CLAUDE.md and oracle_nuisances.py's docstring.
...
# Grid: HARD CELL ONLY -- same as ../ML_nuisance/run_simulation.py.
P_I_VALUES = [0.2]
PH_VALUES = [0.3]
