...
Archived config for the original n=24000 production run (ph in {0.2,0.5,0.8}).
This directory's current production config is run_simulation.py, n=72000,
ph in {0.3,0.5,0.8} -- run this one only to reproduce the historical n=24000
results, which are kept as simulation_results_n24k.{pkl,csv}.

Results are saved to simulation_results_n24k.pkl (and also as
simulation_results_n24k.csv for easy inspection).
...
output_path = os.path.join(os.path.dirname(__file__), "simulation_results_n24k.pkl")
