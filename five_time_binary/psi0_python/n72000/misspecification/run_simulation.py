...
from estimators import GAMMA_ADD  # additive misspecification constant (env-driven)
...
#   (False, False) -> both correct (baseline, reproduces n24000 at these cells)
...
# Output filename tagged by the additive misspecification constant so runs at
# different GAMMA_ADD do not overwrite each other.
output_path = os.path.join(os.path.dirname(__file__),
                           f"simulation_results_add{GAMMA_ADD}.pkl")
print(f"GAMMA_ADD = {GAMMA_ADD}  ->  {os.path.basename(output_path)}")
...
results["add_const"] = GAMMA_ADD  # tag the additive misspecification constant
