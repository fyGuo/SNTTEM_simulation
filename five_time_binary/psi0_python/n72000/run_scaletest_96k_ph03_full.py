"""Full 300-iteration confirmatory run, n=96000, ph=0.3 cells only.

NOT the production run (that's run_simulation.py, n=24000, full 6-cell grid).
Follow-up to run_scaletest_96k_ph02_full.py: the ph=0.2 cells at n=96000
brought psi05's failure rate under check_results.py's 5% threshold, but the
distribution is still heavy-tailed just below the |est|>10 cutoff (see the
psi05_96k_median_variance.png / psi05_96k_vs_24k_failure_rate.png discussion).
This run tests whether ph=0.3 -- more subjects following a constant-treatment
path than ph=0.2, since the three-step weight is a product of four w_t --
stabilizes the ph=0.2 cells further, for the same two p_I values (0.2 and 1).
...
P_I_VALUES = [0.2, 1]
PH_VALUES = [0.3]  # follow-up to the ph=0.2 scale test

HERE = os.path.dirname(__file__)
output_path = os.path.join(HERE, "scaletest_96k_ph03_300iter_results.pkl")
