...
of the fold) -- mirrors the psi1_python/n72000/ML_nuisance fix (see
CLAUDE.md): that subsetting was starving the mu-fit for the sparse
constant-treatment-path corner (ph=0.3, p_I=0.2) and producing catastrophic
Robins'-estimator outliers there under psi1's non-null mechanism. Applied
here too for consistency between the two directories, though psi0's mu is a
constant under the null mechanism so the effect is expected to be smaller.
Prediction plugs in the counterfactual A_t=A_{t-1} value -- an interpolation
within A_t's observed {0,1} support, not extrapolation, since A_t is binary
and both values appear in the full training fold.
"""
import numpy as np
import pandas as pd
